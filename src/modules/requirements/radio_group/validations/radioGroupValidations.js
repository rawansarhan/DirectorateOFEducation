...
const radioGroupBodySchema = Joi.object({
  label: Joi.string()
    .trim()
    .min(1)
    .max(255)
...
function validateCreateRadioGroup (data) {
  const { error, value } = radioGroupBodySchema.validate(data, {
    abortEarly: false,
    allowUnknown: false,
    stripUnknown: true
  })
