...
function formatClientErrorMessage (err) {
  ...
  if (err?.statusCode && err.statusCode < HTTP_STATUS.INTERNAL_SERVER_ERROR && err.message) {
    return err.message
  }
  ...
  if (err.isAxiosError) {
    const remote = err.response?.data?.message || err.response?.data?.error
...
const errorHandler = (err, req, res, next) => {
  ...
  if (err.name === 'ValidationError') {
    return ApiResponder.unprocessableResponse(res, err.message, err.name)
  }
